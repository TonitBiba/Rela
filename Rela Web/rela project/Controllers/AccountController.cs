...
namespace Rela_project.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
                return Redirect(ConfigurationManager.AppSettings["url"] + "Home/AccountConfirmed");
            else
                return GetErrorResult(result);
        ...
        [HttpGet]
        [AllowAnonymous]
        [Route("ResetPasswordView", Name = "ResetPasswordViewRoute")]
        public IHttpActionResult ResetPasswordView(string Email, string code) {
            return Redirect(ConfigurationManager.AppSettings["url"] + "Home/ResetPassword/?Email=" + Email + "&code=" + code);
        }
        ...
        public async Task<IHttpActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest("Model is not valid. Check it");
            }
            var user = await UserManager.FindByEmailAsync(model.Email);
            if (user == null)
            {
                return BadRequest("This user does not exist in the system. Check email again");
            }
            model.Code = model.Code.Replace(" ", "+");
            var result = await UserManager.ResetPasswordAsync(user.Id, model.Code, model.Password);
            if (result.Succeeded)
            {
                return Redirect(ConfigurationManager.AppSettings["url"] + "Home/ResetPasswordConfirmed");
            }
            else
            {
                return BadRequest();
        ...
        [HttpPost]
        [Route("userdata")]
        public IHttpActionResult userdata([FromBody] string username)
        {
            var user = UserManager.FindByName(username);
            return Ok(new { userId = user.Id, FirstName = user.FirstName, LastName = user.LastName, user_image = user.user_image });
        }
        ...
        [Route("checkEmail")]
        [AllowAnonymous]
        public async Task<IHttpActionResult> checkEmail([FromBody] string email) {
